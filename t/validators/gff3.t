...
my $test1_repo = dir()->subdir('t')->subdir('data')->subdir('test1');

{
    my $test1_tmp = File::Temp->newdir;
    system 'cp', '-ra', $test1_repo->children, $test1_tmp;

    # test the job file creation
    my $pipe = CXGN::ITAG::Pipeline->open( basedir => $test1_tmp );
  ...
  SKIP: {
        skip 'validate_gff3.pl not in path, cannot test validation', 5
        ...
        my $queue = $ENV{ITAG_TEST_CLUSTER_QUEUE}
            or skip 'set ITAG_TEST_CLUSTER_QUEUE to test cluster job submission', 3;
...

{
    ...
    is( $analysis->uncached_status($batch), 'validating', 'starts as validating' );

    # test force-validation
    $analysis->force_validate( $batch );

    is( $analysis->uncached_status($batch), 'done', 'analysis now shows done' )
        or diag `find $test1_tmp -ls`;
}
